...
def flash_attn_varlen_func(
    ...
    def can_route_single_seq_to_dense():
        ...
        if cu_seqlens_q.numel() != 2 or cu_seqlens_k.numel() != 2:
            return False
        # The buffer may be LONGER than the sequence: vLLM's chunked-context path gathers
        # into a fixed-size workspace and describes a shorter valid range. Accept >= and
        # slice the valid prefix below. max_seqlen_* is the true length here (one sequence,
        # so the batch max IS its length); rows past it are stale and must not be attended.
    ...
    if can_route_single_seq_to_dense():
        # Slice + unsqueeze are both views (a contiguous [S, H, D] prefix), so no copy.
        r = flash_attn_func(
            q[:max_seqlen_q].unsqueeze(0),
