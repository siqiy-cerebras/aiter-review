...
def _opus_sink(nheads, enabled, hi=False):
    ...
    max-reference path are both exercised within a single case. hi selects a
    dominant range whose upper heads exceed the key-score max by more than exp2
    can represent -- a logit the softmax must rebase onto (max(m_row, sink)) rather
    than reference to the key max, or the denominator overflows to inf.
    """
    if not enabled:
        return None
...
def _run_opus_batch_case(
    ...
    in the softmax denominator: it leaves the numerator but grows the denominator, so
    it attenuates O for rows that see keys and raises their LSE; fully-masked rows stay
    at O=0 with LSE equal to the sink logit. sink_hi makes that logit dominant (larger
    than every key score), which the kernel must handle by rebasing the softmax max
    rather than overflowing exp2.
    """
    torch.manual_seed(0)
    q = torch.randn(
...
# Sink correctness: a representative subset. Two distinct fully-masked paths reach the
# "attend only the sink" branch: sk==0 (zero-tile early return) and causal sq>sk (early
# rows masked after real KV tiles are processed).
_OPUS_SINK_CASES = [
    (2, 64, 64, 8, 2),  # GQA
    (2, 128, 128, 16, 1),  # MQA
    (1, 256, 256, 8, 8),  # MHA
    (2, 300, 0, 8, 2),  # no keys -> every row attends only the sink
    (2, 512, 128, 8, 2),  # causal sq>sk -> early rows masked after real KV tiles
]
_OPUS_SINK_IDS = [
...
@pytest.mark.parametrize(
    "batch_size,seqlen_q,seqlen_kv,nheads,nheads_k",
    [(2, 64, 64, 8, 2), (1, 256, 256, 8, 8)],  # rows that DO see keys
    ids=["b2_sq64_sk64_h8_hkv2", "b1_sq256_sk256_h8_hkv8"],
)
def test_flash_attn_func_opus_sink_dominant(
    batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, causal, d, monkeypatch
):
    """Sink logits large enough to exceed every key score. Rows here see keys, so
    the kernel must rebase the softmax onto max(m_row, sink); referencing the sink
    to the key max instead overflows exp2 and yields lse=inf. Guards that path --
    the fp32 logaddexp reference stays finite, so an overflowed lse fails the check.
    """
    if get_gfx() != "gfx950":
        pytest.skip("opus symmetric kernel requires gfx950")
    monkeypatch.setenv("AITER_ENABLE_FMHA_OPUS", "1")
...
@pytest.mark.parametrize("d", [64, 128])
def test_flash_attn_func_opus_sink_gate_matches_launcher(d, monkeypatch):
    """A one-element ([H=1]) sink with non-unit stride is 'contiguous' to PyTorch but
    the OPUS launcher requires stride==1. The gate must mirror that and decline OPUS
    (fall back) rather than select it and then throw; the result still matches the ref.
    """
    if get_gfx() != "gfx950":
        pytest.skip("opus symmetric kernel requires gfx950")
    monkeypatch.setenv("AITER_ENABLE_FMHA_OPUS", "1")
    torch.manual_seed(0)
    q = torch.randn(2, 64, 1, d, device="cuda", dtype=dtypes.bf16)
    k = torch.randn(2, 64, 1, d, device="cuda", dtype=dtypes.bf16)
    v = torch.randn(2, 64, 1, d, device="cuda", dtype=dtypes.bf16)
    sink = torch.randn(2, device="cuda", dtype=torch.float32)[::2]  # [1], stride 2
    assert (
        sink.is_contiguous() and sink.stride(0) != 1
    )  # the layout that fooled the gate
    with torch.no_grad():
        out, _ = aiter.flash_attn_func(
            q, k, v, softmax_scale=None, causal=False, return_lse=True, sink_ptr=sink
        )
    out_ref, _, _ = attention_ref(q, k, v, causal=False, sink=sink)
    diff = (out - out_ref).abs().max().item()
    print(f"[opus-sink-gate d{d}] out max diff: {diff}")
    assert diff <= 0.02


@pytest.mark.parametrize("d", [64, 128])
def test_flash_attn_func_opus_sink_below_sentinel_no_keys(d, monkeypatch):
    """A no-key row (sk=0) with a finite sink far below the kernel's -1e30 row-max
    sentinel must attend only the sink: O = 0 and LSE = that finite sink, not -inf.
    The l_row == 0 branch sets the result directly; a rebase against the sentinel
    instead would drop the sub-sentinel sink and emit lse = -inf."""
    if get_gfx() != "gfx950":
        pytest.skip("opus symmetric kernel requires gfx950")
    monkeypatch.setenv("AITER_ENABLE_FMHA_OPUS", "1")
    torch.manual_seed(0)
    ...
    k = torch.randn(2, 0, nheads_k, d, device="cuda", dtype=dtypes.bf16)
    v = torch.randn(2, 0, nheads_k, d, device="cuda", dtype=dtypes.bf16)
    sink = torch.full((nheads,), -1e31, device="cuda", dtype=torch.float32)
    with torch.no_grad():
        out, lse = aiter.flash_attn_func(
            q, k, v, softmax_scale=None, causal=False, return_lse=True, sink_ptr=sink
        )
    assert (out == 0).all(), "no keys -> O must be 0"
    assert torch.isfinite(lse).all(), "sub-sentinel sink must give finite LSE, not -inf"
    torch.testing.assert_close(
        lse, sink.view(1, nheads, 1).expand_as(lse), rtol=1e-3, atol=0.0
    )
...
def test_flash_attn_func_opus_sink_posinf(causal, d, monkeypatch):
    """A +inf sink: every row attends only the sink, so O == 0 and LSE == +inf.
    Guards the inf - inf indeterminate in the finalization (a NaN there emits -inf)."""
    if get_gfx() != "gfx950":
        pytest.skip("opus symmetric kernel requires gfx950")
    monkeypatch.setenv("AITER_ENABLE_FMHA_OPUS", "1")
    torch.manual_seed(0)
...
def _run_opus_single_seq_varlen(
    ...
    with torch.no_grad():
        ...
        # The same single sequence sent straight to the dense OPUS op, LSE included.
    ...
    # Routing + view correctness: both O and LSE must equal the dense path exactly
    # (LSE feeds chunk-state merging, so a wrong squeeze/route would corrupt it).
    assert torch.equal(
        out, out_dense.squeeze(0)
    ), f"{tag}: single-seq varlen did not route to the dense OPUS kernel"
    assert torch.equal(
        lse, lse_dense.squeeze(0)
    ), f"{tag}: single-seq varlen LSE != dense OPUS LSE"
